...
for x in xrange(len(sys.argv)-8):
    ...
    my_proc.set_ana_output_file("SNoutput_ana.root") #this is the output_ana root file, third from last entry in the command line
    my_proc.set_output_file(sys.argv[-7])      #sys.argv[-7])   #this is the compressed waveforms root file, second from last entry in the command line

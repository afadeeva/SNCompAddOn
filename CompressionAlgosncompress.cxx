...
/*
  ...
  for (size_t m = 0; m < waveform.size(); m++) {
     _thisTick = _begin + m;
     _nextTick = _begin + m + 1;
*/
       if ( nextTick - thisTick == 0 ){ postHuffmanTicks =+ 1; }
       else if ( nextTick - thisTick ==-1 ){
	 if (postHuffmanTicks + 2 < 12 ) { postHuffmanTicks =+ 2; }
	 else{ postHuffwords =+ 1;}}
       else if ( nextTick - thisTick == 1 ){ 
	if (postHuffmanTicks + 3 < 12 ) { postHuffmanTicks =+ 3; }
	 else{ postHuffwords =+ 1; }}
       else if ( nextTick - thisTick ==-2 ){ 
	if (postHuffmanTicks + 4 < 12 ) { postHuffmanTicks =+ 4; }
	 else{ postHuffwords =+ 1; }}
       else if ( nextTick - thisTick == 2 ){ 
	if (postHuffmanTicks + 5 < 12 ) { postHuffmanTicks =+ 5; }
	 else{ postHuffwords =+ 1; }}
       else if ( nextTick - thisTick ==-3 ){ 
	if (postHuffmanTicks + 6 < 12 ) { postHuffmanTicks =+ 6; }
	 else{ postHuffwords =+ 1; }}
       else if ( nextTick - thisTick == 3 ){ 
	if (postHuffmanTicks + 7 < 12 ) { postHuffmanTicks =+ 7; }
	 else{ postHuffwords =+ 1; }}
       //else if ( nextADC - thisADC == 0 ){ }
//  }
